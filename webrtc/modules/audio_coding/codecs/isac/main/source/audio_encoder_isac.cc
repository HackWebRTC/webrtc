...
AudioEncoderDecoderIsac::Config::Config()
    : sample_rate_hz(16000), frame_size_ms(30), bit_rate(32000) {}
...
int AudioEncoderDecoderIsac::Num10MsFramesInNextPacket() const {
  CriticalSectionScoped cs(lock_.get());
  int samples_in_next_packet = WebRtcIsac_GetNewFrameLen(isac_state_);
  return DivExact(samples_in_next_packet, DivExact(sample_rate_hz(), 100));
}

bool AudioEncoderDecoderIsac::Encode(uint32_t timestamp,
                                     const int16_t* audio,
  ...
  *encoded_bytes = r;
  if (r > 0) {
    ...
    packet_in_progress_ = false;
    *encoded_timestamp = packet_timestamp_;
  }
  return true;
}
