...
int ViEAutoTest::ViECustomCall()
{
    ...
    numberOfErrors += ViETest::TestError(ptrVE != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

	VoEBase*	ptrVEBase = VoEBase::GetInterface(ptrVE);
	numberOfErrors += ViETest::TestError(ptrVEBase != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
	...
	numberOfErrors += ViETest::TestError(ptrVECodec != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

	VoEHardware* ptrVEHardware = VoEHardware::GetInterface(ptrVE);
	numberOfErrors += ViETest::TestError(ptrVEHardware != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

	VoEAudioProcessing*	ptrVEAPM = VoEAudioProcessing::GetInterface(ptrVE);
	numberOfErrors += ViETest::TestError(ptrVEAPM != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
    ...
    numberOfErrors += ViETest::TestError(ptrViE != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

    ViEBase* ptrViEBase = ViEBase::GetInterface(ptrViE);
    numberOfErrors += ViETest::TestError(ptrViEBase != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

	error = ptrViEBase->Init();
    numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

	ViECapture* ptrViECapture = ViECapture::GetInterface(ptrViE);
    numberOfErrors += ViETest::TestError(ptrViECapture != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

	ViERender* ptrViERender = ViERender::GetInterface(ptrViE);
    numberOfErrors += ViETest::TestError(ptrViERender != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

    ViECodec* ptrViECodec = ViECodec::GetInterface(ptrViE);
    numberOfErrors += ViETest::TestError(ptrViECodec != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

	ViENetwork* ptrViENetwork = ViENetwork::GetInterface(ptrViE);
    numberOfErrors += ViETest::TestError(ptrViENetwork != NULL, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
	...
	while(1)
	{
		...
		GetAudioDevices(ptrVEBase, ptrVEHardware, audioCaptureDeviceName, audioCaptureDeviceIndex, audioPlaybackDeviceName, audioPlaybackDeviceIndex);
		...
		// start the call now
		PrintCallInformation(ipAddress,
			deviceName, uniqueId, videoCodec, videoTxPort, videoRxPort,
			audioCaptureDeviceName, audioPlaybackDeviceName, audioCodec, audioTxPort, audioRxPort);

		std::cout << std::endl;
		std::cout << "1. Start the call" << std::endl;
		std::cout << "2. Reconfigure call settings" << std::endl;
		std::cout << "3. Go back to main menu" << std::endl;
		std::cout << "What do you want to do? Press enter for default (Start the call): ";
		...
		{
			// invalid selection
			std::cout << "ERROR: Code=" << error << " Invalid selection" << std::endl;
			continue;
		}
	}
	...
	if(startCall == true)
	{
		...
		error = ptrVEBase->SetSendDestination(audioChannel, audioTxPort, ipAddress);
		numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
		...
		error = ptrVECodec->SetVADStatus(audioChannel, true, webrtc::kVadAggressiveHigh );
		numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
		...
		error = ptrViERtpRtcp->SetKeyFrameRequestMethod(videoChannel, kViEKeyFrameRequestPliRtcp);
		numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
		...
		error = ptrViERender->AddRenderer(videoChannel,  _window2, 1, 0.0, 0.0, 1.0, 1.0);
		numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);

		error = ptrViENetwork->SetSendDestination(videoChannel, ipAddress, videoTxPort);
		numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
		...
		numberOfErrors += ViETest::TestError(remainingInterfaces == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
		...
		numberOfErrors += ViETest::TestError(deleted == true, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
...
bool ViEAutoTest::GetVideoDevice(ViEBase* ptrViEBase, ViECapture* ptrViECapture, char* captureDeviceName, char* captureDeviceUniqueId)
{
	int error = 0;
	int numberOfErrors = 0;
	...
	while(1)
	{
		...
		for (captureIdx = 0; captureIdx < ptrViECapture->NumberOfCaptureDevices(); captureIdx++)
		{
			...
			numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
			std::cout << "   " << captureIdx+1 << ". " << deviceName << std::endl;
		}
		std::cout << "Choose a capture device. Press enter for default (" << deviceName << "/" << uniqueId << "): ";
		std::getline(std::cin, str);
		...
		if(captureDeviceIndex == 0)
		{
			...
			error = ptrViECapture->GetCaptureDevice(0, deviceName, KMaxDeviceNameLength, uniqueId, KMaxUniqueIdLength);
			numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
			strcpy(captureDeviceUniqueId, uniqueId);
			strcpy(captureDeviceName, deviceName);
			return true;
		}
		else if(captureDeviceIndex < 0 || captureDeviceIndex > (int)ptrViECapture->NumberOfCaptureDevices())
		{
			// invalid selection
			continue;
		}
		...
		{
			error = ptrViECapture->GetCaptureDevice(captureDeviceIndex - 1, deviceName, KMaxDeviceNameLength, uniqueId, KMaxUniqueIdLength);
			numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
...
bool ViEAutoTest::GetAudioDevices(VoEBase* ptrVEBase, VoEHardware* ptrVEHardware,
								   ...
								   char* playbackDeviceName, int& playbackDeviceIndex)
{
	...
	while(1)
	{
		...
		for (captureIdx = 0; captureIdx < numberOfRecordingDevices ; captureIdx++)
		{
			memset(recordingDeviceName, 0, KMaxDeviceNameLength);
			memset(recordingDeviceUniqueName, 0, KMaxDeviceNameLength);
			error = ptrVEHardware->GetRecordingDeviceName(captureIdx, recordingDeviceName, recordingDeviceUniqueName);
			numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
			std::cout << "   " << captureIdx+1 << ". " << recordingDeviceName << std::endl;
		}

		std::cout << "Choose an audio capture device. Press enter for default (" << recordingDeviceName << "): ";
		std::getline(std::cin, str);
		...
		if(captureDeviceIndex == 0)
		{
			...
			error = ptrVEHardware->GetRecordingDeviceName(recordingDeviceIndex, recordingDeviceName, recordingDeviceUniqueName);
			numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
			break;
		}
		else if(captureDeviceIndex < 0 || captureDeviceIndex > numberOfRecordingDevices)
		{
			// invalid selection
			continue;
		}
	...
	while(1)
	{
		...
		for (captureIdx = 0; captureIdx < numberOfPlaybackDevices ; captureIdx++)
		{
			memset(playbackDeviceName, 0, KMaxDeviceNameLength);
			memset(playbackDeviceUniqueName, 0, KMaxDeviceNameLength);
			error = ptrVEHardware->GetPlayoutDeviceName(captureIdx, playbackDeviceName, playbackDeviceUniqueName);
			numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
			std::cout << "   " << captureIdx+1 << ". " << playbackDeviceName << std::endl;
		}

		std::cout << "Choose an audio playback device. Press enter for default (" << playbackDeviceName << "): ";
		std::getline(std::cin, str);
		int captureDeviceIndex = atoi(str.c_str());
		...
		if(captureDeviceIndex == 0)
		{
			...
			error = ptrVEHardware->GetPlayoutDeviceName(playbackDeviceIndex, playbackDeviceName, playbackDeviceUniqueName);
			numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d", __FUNCTION__, __LINE__);
			return true;
		}
		else if(captureDeviceIndex < 0 || captureDeviceIndex > numberOfPlaybackDevices)
		{
			// invalid selection
			continue;
		}
...
bool ViEAutoTest::GetIPAddress(char* iIP)
{
	...
	{
		std::cout << std::endl;
		std::cout << "Enter destination IP. Press enter for default (" << oIP << "): ";
		std::getline(std::cin, str);
...
bool ViEAutoTest::GetVideoPorts(int* txPort, int* rxPort)
{
	...
	*rxPort = DEFAULT_VIDEO_PORT;
	...
	while(1)
	{
		std::cout << "Enter video send port. Press enter for default (" << *txPort << ")";
		std::getline(std::cin, str);
		port = atoi(str.c_str());
	...
	while(1)
	{
		std::cout << "Enter video receive port. Press enter for default (" << *rxPort << ")";
		std::getline(std::cin, str);
		port = atoi(str.c_str());
...
bool ViEAutoTest::GetVideoCodec(ViECodec* ptrViECodec, webrtc::VideoCodec& videoCodec)
{
	int error = 0;
	int numberOfErrors = 0;
	...
	while(!exitLoop)
	{
		...
		for (codecIdx = 0; codecIdx < ptrViECodec->NumberOfCodecs(); codecIdx++)
		{
			...
			std::cout << "   " << codecIdx+1 << ". " << videoCodec.plName << std::endl;
		}
		std::cout << std::endl;
		std::cout << "Choose video codec. Press enter for default (" << DEFAULT_VIDEO_CODEC << ")";
		std::getline(std::cin, str);
...
bool ViEAutoTest::GetAudioPorts(int* txPort, int* rxPort)
{
	...
	*rxPort = DEFAULT_AUDIO_PORT;
	...
	while(1)
	{
		std::cout << "Enter audio send port. Press enter for default (" << *txPort << ")";
		std::getline(std::cin, str);
		port = atoi(str.c_str());
	...
	while(1)
	{
		std::cout << "Enter audio receive port. Press enter for default (" << *rxPort << ")";
		std::getline(std::cin, str);
		port = atoi(str.c_str());
...
bool ViEAutoTest::GetAudioCodec(webrtc::VoECodec* ptrVeCodec, webrtc::CodecInst& audioCodec)
{
	...
	while(1)
	{
		...
		for (codecIdx = 0; codecIdx < ptrVeCodec->NumOfCodecs(); codecIdx++)
		{
			...
			std::cout << "   " << codecIdx+1 << ". " << audioCodec.plname << std::endl;
		}
		std::cout << std::endl;
		std::cout << "Choose audio codec. Press enter for default (" << DEFAULT_AUDIO_CODEC << ")";
		std::getline(std::cin, str);
		codecSelection = atoi(str.c_str());
...
void ViEAutoTest::PrintCallInformation(char* IP,
	char* videoCaptureDeviceName, char* videoCaptureUniqueId, webrtc::VideoCodec videoCodec, int videoTxPort, int videoRxPort,
	char* audioCaptureDeviceName, char* audioPlaybackDeviceName, webrtc::CodecInst audioCodec, int audioTxPort, int audioRxPort)
{
	std::string str;
	...
	std::cout << "\tVideo Capture Device: " << videoCaptureDeviceName << std::endl;
	...
	std::cout << "\tAudio Capture Device: " << audioCaptureDeviceName << std::endl;
	std::cout << "\tAudio Playback Device: " << audioPlaybackDeviceName << std::endl;
