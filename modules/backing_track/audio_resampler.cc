...
AudioResampler::AudioResampler(AVSampleFormat input_format,
    ...
    if (output_channel_num_ == 1) {
        output_channel_layout = AV_CHANNEL_LAYOUT_MONO;
    } else {
        output_channel_layout = AV_CHANNEL_LAYOUT_STEREO;
    }
    ...
    av_opt_set_chlayout(context_.get(), "out_chlayout", &output_channel_layout, 0);
...
int32_t AudioResampler::CalcOutputSize(int32_t input_size) {
    if (input_channel_num_ <= 0
        || av_get_bytes_per_sample(input_format_) <= 0) {
        return kMixerErrResamplerInit2;
    }
