...
Recorder::~Recorder() {
    ...
    record_queue_.get_deleter()(record_queue_.get());
    record_queue_.release();
    Stop();
}
...
int Recorder::parseParamSets(int video_codec_id, const uint8_t* payload, uint32_t length) {
    ...
    if (video_codec_id == (int) AV_CODEC_ID_H264) {
        ...
        std::vector<H265::NaluIndex> nalu_indices = H265::FindNaluIndices(rtc::ArrayView<const uint8_t>(payload, length));
        bool is_param_sets = false;
